...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        h_norm = self.norm(x)
        qkv = self.qkv(h_norm)
        # Bolt Optimization: Batch reshape and transpose on the combined qkv tensor
        # before unbinding to reduce PyTorch dispatcher overhead and non-contiguous memory allocations.
        qkv_reshaped = qkv.reshape(b, 3, c, h * w).transpose(-2, -1).unsqueeze(2)
        q, k, v = qkv_reshaped.unbind(dim=1)
