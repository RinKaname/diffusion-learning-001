...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        h_norm = self.norm(x)
        qkv = self.qkv(h_norm)
        # Batch reshape and transpose operations on the combined qkv tensor before splitting
        # Reshaping to (b, 1, seq_len, head_dim) for F.scaled_dot_product_attention
        # Assuming single head attention where head_dim = c
        q, k, v = qkv.reshape(b, 3, c, h * w).transpose(-2, -1).unsqueeze(2).unbind(1)
