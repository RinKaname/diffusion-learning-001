...
## 2024-06-16 - Batching PyTorch Tensor Transformations
**Learning:** In PyTorch, applying tensor operations like `reshape`, `transpose`, and `unsqueeze` to individual split tensors (e.g., `q, k, v`) introduces unnecessary overhead. Batching these operations on the combined tensor before splitting (e.g., `unbind`) reduces Python dispatcher overhead and speeds up the module.
**Action:** When reshaping `qkv` tensors for attention, always apply the dimension transformations to the combined `qkv` tensor first, and then split it using `unbind` or `chunk`.
