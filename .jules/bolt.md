...
## 2024-05-24 - Batching Tensor Operations Before Unbinding / Chunking
**Learning:** Applying tensor operations like `reshape` and `transpose` to a combined `qkv` tensor before splitting it via `unbind` or `chunk` significantly reduces PyTorch dispatcher overhead and avoids expensive memory allocations caused by reshaping non-contiguous tensors.
**Action:** When working with combined tensors like `qkv` in PyTorch, reshape and transpose the combined tensor first, then split it along the combined dimension instead of splitting first and repeating the operations on each chunk.
