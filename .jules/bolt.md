...
## 2024-05-24 - Batching Tensor Operations in Attention Blocks
**Learning:** In PyTorch attention modules, batching tensor operations like `reshape`, `transpose`, and `unsqueeze` on the combined `qkv` tensor *before* splitting it with `unbind` or `chunk` significantly reduces dispatcher overhead compared to splitting first and operating on `q`, `k`, and `v` individually.
**Action:** Apply tensor shape manipulations to the combined `qkv` tensor before unbinding them into separate tensors to optimize PyTorch execution speed.
