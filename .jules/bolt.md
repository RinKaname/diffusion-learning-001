...
## 2024-05-24 - Batching Tensor Operations in PyTorch Attention
**Learning:** Applying tensor operations like `reshape` and `transpose` to individual `q`, `k`, and `v` tensors after splitting causes significant PyTorch dispatcher overhead. Batching these operations on the combined `qkv` tensor before using `unbind` reduces the number of operations and makes the attention module much faster.
**Action:** Always batch tensor transformations (e.g., `reshape`, `transpose`, `unsqueeze`) on the combined `qkv` tensor before separating them using `unbind(dim)`. This pattern significantly reduces dispatcher overhead.
