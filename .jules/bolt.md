...
## 2024-06-29 - QKV Tensor Operations Batching
**Learning:** In PyTorch, applying tensor operations like `reshape` and `transpose` individually to `q`, `k`, and `v` tensors after chunking a combined `qkv` tensor introduces unnecessary dispatcher overhead and triggers expensive memory allocations due to handling non-contiguous chunks.
**Action:** When implementing attention modules, apply reshaping and transposing to the unified `qkv` tensor first, and then use `unbind` to separate them, rather than splitting first and processing them individually.
