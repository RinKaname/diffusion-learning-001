...
## 2024-05-24 - Batch Tensor Operations Before Splitting
**Learning:** When preparing combined `qkv` tensors for attention (e.g., in `SelfAttention`), splitting the tensor into `q`, `k`, `v` using `.chunk()` and then applying identical operations (`reshape`, `transpose`, `unsqueeze`) to each piece individually incurs redundant PyTorch dispatcher overhead. Batching these operations on the combined tensor first and then using `.unbind()` is significantly faster.
**Action:** Apply tensor reshaping and transposing to the unified `qkv` tensor first, then separate them with `.unbind(dim=...)` to reduce PyTorch dispatcher overhead and improve forward pass performance.
