...
## 2024-06-18 - Batching Tensor Operations in Attention Blocks
**Learning:** In PyTorch attention module implementations, applying tensor operations like `reshape`, `transpose`, and `unsqueeze` to the combined `qkv` tensor before using `unbind` or `chunk` is more efficient than splitting the tensor first and applying operations to `q`, `k`, and `v` individually. It significantly reduces PyTorch dispatcher overhead and speeds up the forward pass.
**Action:** Apply tensor reshaping operations to the combined `qkv` tensor before unbinding when possible.
